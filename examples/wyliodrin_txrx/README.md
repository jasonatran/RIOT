# wyliodrin_txrx example

Before trying to understand this example, see the RIOT tutorials on their Github
pages (such as here: https://github.com/RIOT-OS/Tutorials) to get the 
hello-world example up and running. Then, run the wyliodrin_txrx example (or the
gnrc_networking example) and explore the shell commands. You can do many things
from the shell such as changing the channel and HW address of the zigbee
transceiver. You can also explore how to send/receive UDP packets from the RIOT
tutorials (see task 6).

Once you are warmed up, read the Makefile in this example and start reading the
code starting with main.c. The exercise for students should probably not use the
